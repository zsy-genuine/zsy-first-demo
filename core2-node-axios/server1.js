...
let url=require('url')
//url 解析url上面的各种参数   url是请求传过来的   自带
//http 模块 自带模块
// 创建自己的第一个服务器
// createServer  创建服务器
//req request  请求
//res response 响应
// 需要有端口号  baidu：80   0-65000
// listen 表示监听的是哪个端口号
// res.end()后端响应数据
// vscode  coderuning 插件启动  webstorm  直接运行
// 命令行启动 node 文件名
//注意 服务器端代码有改动需要重启服务器
//urlobj.pathname  访问路径  localhost:3000/ localhost:3000/a  /a
...
// 文件 文件夹起名 不要用中文 node vue react axios 已经是第三方的名字